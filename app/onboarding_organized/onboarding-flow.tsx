/**
 * OnboardingFlow — Organized variant
 *
 * Mirrors `/onboarding_early` exactly (same structure, same atoms, same
 * Server Action, same DB column) but with angle-specific Q4 options and
 * an intro sub-line aimed at producers who feel scattered, not time-pressed.
 *
 * Q1 producer name → Q2 email → Q3 work URL → Q4 "What slows you down today?"
 *   with organize-specific multi-select options → submit → final.
 *
 * The Q4 options are distinct enough from the /onboarding_early set that
 * we can tell which landing the lead came through by inspecting their
 * grow_goals row in Supabase — no schema change needed.
 */
...
import { submitOnboarding } from "@/app/onboarding_early/actions";

const TOTAL_QUESTIONS = 4;
const TRANSITION_MS = 700;

/** Pain-point options for the organize angle. Distinct from the /onboarding_early
 *  set so the lead's row in Supabase tells us which landing they came through. */
const PAIN_POINTS = [
  "Finding the right artist for a beat",
  "Following up on DMs I sent",
  "Remembering who got what beat",
  "Sorting through scattered messages",
  "Keeping the contact list updated",
  "Everything — total chaos",
  "Not sure yet",
] as const;
...
/* ============ atoms (identical to /onboarding_early) ============ */
...
function IntroScreen({ onStart }: { onStart: () => void }) {
  return (
    <div className="flex flex-col items-center gap-[24px] text-center motion-safe:animate-in motion-safe:fade-in motion-safe:duration-500 sm:gap-[28px]">
      ...
      <p className="max-w-[46ch] text-pretty text-[clamp(15px,1.3vw,17px)] leading-[1.55] text-text-2">
        Four quick questions so we can understand where your contact chaos
        hurts most — and where Wavloops should start cleaning up.
      </p>
      <p className="font-mono text-[10px] uppercase tracking-[0.13em] text-text-3">
        ...
        <span aria-hidden className="mx-[6px] text-line-strong">·</span>{" "}
  ...
  return (
    <div className="flex flex-col items-center gap-[24px] text-center motion-safe:animate-in motion-safe:fade-in motion-safe:duration-500 sm:gap-[28px]">
      ...
      <div className="mt-[8px]">
        <Link
          href="/organized"
          className="wv-btn wv-btn-ghost wv-btn-lg group"
        >
          Back to site
          <span
            aria-hidden
...
export function OnboardingFlow() {
  ...
  return (
    <div className="relative flex min-h-screen flex-col overflow-hidden bg-bg">
      {/* same soft glow + masked grid as /onboarding_early */}
      <div
        aria-hidden
